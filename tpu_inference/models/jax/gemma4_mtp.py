...
class Gemma4MTPAttention(JaxModule):
    ...
    def __call__(
        ...
        # Group query heads by GQA groups
        q = q.reshape(num_tokens, self.num_kv_heads, num_queries_per_kv,
                      self.head_dim)
        q = jnp.swapaxes(q, 1, 2)
        q = q.reshape(num_tokens, num_queries_per_kv,
                      self.num_kv_heads * self.head_dim)

        # Apply the grouped query normalizer
        q = self.q_norm(q)

        # Restore original shape
        q = q.reshape(num_tokens, num_queries_per_kv, self.num_kv_heads,
                      self.head_dim)
        q = jnp.swapaxes(q, 1, 2)
